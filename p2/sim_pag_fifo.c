...
int choose_page_to_be_replaced (ssystem * S)
{
    ...
    victim = S->frt[frame].page; //the victim is the page, that occupy the first frame from the busy(?)list
    S->listoccupied= S->pgt[victim].frame;	//occupy the frame again that goes from the first to the last of the list

    if (S->detailed)
        printf ("@ Choosing trough the FIFO P%d of M%d for "
                "replacing it\n", victim, frame);
...
{   
    ...
    if(S->listoccupied==-1){
        S->frt[frame].next=frame;//if we have one frame in the list, we put it in the next frame 
    }
    else{
        S->frt[frame].next=S->frt[S->listoccupied].next;//metemos el frame en el primero de la lista de ocupados
...
void print_replacement_report (ssystem * S)
{
    ...
    for(int k=0; k<S->numframes; k++){
        ...
        if(z==1)
       	   printf ("(The complete frames  %8d  which contains the page %8d\n", k, S->frt[k].page); 
    }
